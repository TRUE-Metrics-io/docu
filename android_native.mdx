...
description: 'Complete guide for integrating Truemetrics SDK 1.6.0 into Android applications'
---
...
Uninitialized ──► Initializing ──► Initialized
                       ...
                       │    (init() ran in the background; recovers by itself when a
                       │     screen next reaches the foreground)
                       │
...
suspend fun awaitSdkReady(): String {
    ...
    val ready = sdk.observeSdkStatus().first { status ->
        ...
        // Every error is final except this one: Android refused to start the foreground
        // service because the app was in the background, and the SDK clears it by itself
        // once one of your screens comes to the foreground. Treating it as a failure gives
        // up on an initialization that was about to succeed.
...
<Warning>
  If your app has no screen to bring to the foreground — a headless or MDM-managed
  integration — there is no moment for the SDK to retry at. Call `init()` again from a place
  Android allows a foreground service to start, such as an activity or an exempt receiver.
</Warning>
...
- `startRecording()` — safe to call immediately after `init()`. If the service is not yet bound, the request is queued and executed once binding completes. It is also kept, and begins by itself, when Android refused the foreground-service start — see `FOREGROUND_SERVICE_START_REFUSED` below. If initialization has failed for another reason, the call reports `Status.Error(CONFIG_ERROR)` unless a more specific error is already being reported, which is then left in place; if the SDK was never initialized, `Status.Error(NOT_INITIALIZED)`. To avoid these, await `Status.Initialized` before calling it, or configure an auto-start (see [Recording lifecycle](#recording-lifecycle)).
...
| `Status.Error(FOREGROUND_SERVICE_START_REFUSED)` | **Transient.** `init()` ran while the app was in the background and Android refused to start the foreground service | None. The SDK retries when one of your screens next reaches the foreground, and a `startRecording()` issued meanwhile is kept and begins then. Do **not** treat it as fatal. An app with no screen retries by calling `init()` again from an allowed context. |
...
| FOREGROUND_SERVICE_START_REFUSED | Android refused to start the foreground service because the app was in the background; transient, the SDK retries when a screen next reaches the foreground |
