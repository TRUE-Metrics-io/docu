...
description: 'Complete guide for integrating Truemetrics SDK 1.6.3 into Android applications'
---
...
```gradle
dependencies {
    implementation 'io.truemetrics:truemetricssdk:1.6.3'
}
```
...
<Note>
  ...
  **`location`** and **`dataSync`** foreground-service types (previously `location` + `specialUse`). In the
  ...
  listed **"Special use"** for a Truemetrics service, replace it with **"Data sync"** (keep **"Location"**).
</Note>
...
Uninitialized ──► Initializing ──► Initialized
                       ...
                       │
                       ├──► Error(FOREGROUND_SERVICE_START_REFUSED) ──► Initialized
                       │    (init() ran in the background; the SDK retries when
                       │     the app's UI comes to the foreground)
                       │
...
<Note>
There is no hard timeout on initialization. Under poor connectivity the SDK stays in `Status.Initializing` and keeps retrying. Do not impose an arbitrary client-side timeout — either wait for `Status.Initialized` or a `Status.Error` whose `errorCode` is not `FOREGROUND_SERVICE_START_REFUSED` (that one clears itself, see below), or gate UI on `getDeviceId()` becoming non-null.
</Note>
...
suspend fun awaitSdkReady(): String {
    ...
    val ready = sdk.observeSdkStatus().first { status ->
        ...
        // Not a failure: Android refused the foreground service because the app was
        // in the background. The SDK retries when the UI appears, so keep waiting.
        (status is Status.Error &&
            status.errorCode != ErrorCode.FOREGROUND_SERVICE_START_REFUSED)
    }
...
<Warning>
  If your app has no UI — headless, or driven by an MDM — it never comes to the foreground,
  so the SDK gets no chance to retry. Call `deinitialize()` and then `init()` again from a
  context Android allows, such as a receiver it exempts, and then `startRecording()` again —
  `deinitialize()` drops a pending request.
</Warning>
...
- `startRecording()` — safe to call immediately after `init()`. The request is kept and runs once the service is up: both while the service is still binding, and when Android refused to start it. If initialization failed for some other reason, the call reports `Status.Error(CONFIG_ERROR)` — or leaves a more specific error in place, if one is already reported. If the SDK was never initialized, it reports `Status.Error(NOT_INITIALIZED)`. To avoid these, wait for `Status.Initialized`, or configure an auto-start (see [Recording lifecycle](#recording-lifecycle)).
...
| `Status.Error(NOT_INITIALIZED)` | `startRecording()` was called without an initialized SDK — before `init()`, or after `deinitialize()` | Call `init()` first. The request is not held; call `startRecording()` again once initialized. |
...
| `Status.Error(FOREGROUND_SERVICE_START_REFUSED)` | **Transient.** `init()` ran in the background, where Android refuses to start a foreground service | None. The SDK retries when your UI comes to the foreground, and a `startRecording()` made in the meantime runs then. If your app has no UI, call `deinitialize()` and then `init()` again from an exempt receiver, then `startRecording()` again — `deinitialize()` drops a pending request. |
...
```kotlin
// Start as soon as init() completes
val config = SdkConfiguration.Builder("YOUR_API_KEY")
    .delayAutoStartRecording(SdkConfiguration.AUTO_START_ON_INIT)
    .build()

// Or start after a delay
val delayed = SdkConfiguration.Builder("YOUR_API_KEY")
    .delayAutoStartRecording(30_000L)                       // milliseconds
    .build()

val units = SdkConfiguration.Builder("YOUR_API_KEY")
    .delayAutoStartRecording(2, TimeUnit.MINUTES)
    .build()
```

While a configured delay is pending, the SDK reports `Status.DelayedStart`. Calling
...
<Note>
**Changed in 1.6.0.** Releases 1.3.0 through 1.5.x defaulted to `AUTO_START_ON_INIT`, so an app that
...
`deinitialize()`; every launch after that follows the new default.
</Note>
...
| FOREGROUND_SERVICE_START_REFUSED | `init()` ran in the background, where Android refuses to start a foreground service. Temporary: the SDK retries when the app's UI comes to the foreground |
...
| NOT_INITIALIZED | `startRecording()` was called before `init()`, so there is nothing to record with |
