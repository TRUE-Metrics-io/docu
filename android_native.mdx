...
Uninitialized ──► Initializing ──► Initialized
                       ...
                       │    (init() ran in the background; clears when a screen
                       │     next reaches the foreground)
                       │
...
suspend fun awaitSdkReady(): String {
    ...
    val ready = sdk.observeSdkStatus().first { status ->
        ...
        // The one error that is not final: the SDK clears it itself once a screen
        // reaches the foreground. Failing here gives up on a working session.
...
<Warning>
  An app with no screen — headless or MDM-managed — never gives the SDK a moment to retry at.
  Call `init()` again from an activity or an exempt receiver instead.
</Warning>
...
- `startRecording()` — safe to call immediately after `init()`. If the service is not yet bound, or Android refused the foreground-service start, the request is kept and runs once the service is up. If initialization failed for another reason the call reports `Status.Error(CONFIG_ERROR)`, unless a more specific error is already reported — that one stays; if the SDK was never initialized, `Status.Error(NOT_INITIALIZED)`. To avoid these, await `Status.Initialized` first, or configure an auto-start (see [Recording lifecycle](#recording-lifecycle)).
...
| `Status.Error(FOREGROUND_SERVICE_START_REFUSED)` | **Transient.** `init()` ran in the background; Android refused the foreground service | None — the SDK retries when a screen next reaches the foreground, and a `startRecording()` issued meanwhile begins then. An app with no screen: call `init()` again from an activity or exempt receiver. |
...
| FOREGROUND_SERVICE_START_REFUSED | `init()` ran in the background and Android refused the foreground service; transient, the SDK retries when a screen reaches the foreground |
