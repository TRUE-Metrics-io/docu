...
- `startRecording()` — safe to call immediately after `init()`. If the service is not yet bound, the request is queued and executed once binding completes. If initialization has failed, the call transitions the SDK to `Status.Error(CONFIG_ERROR)`; if the SDK was never initialized, to `Status.Error(NOT_INITIALIZED)`. To avoid the first, await `Status.Initialized` before calling it, or configure an auto-start (see [Recording lifecycle](#recording-lifecycle)).
