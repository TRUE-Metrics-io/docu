# Truemetrics Android SDK - Usage Guide
...
### Auto-Start Recording Configuration

The SDK can automatically start recording after initialization with configurable delays:
...
// Start recording after 30 seconds
val config = SdkConfiguration.Builder(apiKey)
    .delayAutoStartRecording(SdkConfiguration.DELAY_30_SECONDS)
    .build()

// Start recording after 2 minutes (using TimeUnit)
val config = SdkConfiguration.Builder(apiKey)
    .delayAutoStartRecording(2, TimeUnit.MINUTES)
    .build()

// Disable auto-start completely (manual control only)
val config = SdkConfiguration.Builder(apiKey)
    .delayAutoStartRecording(SdkConfiguration.NEVER)
    .build()
```
