...
traces where possible. They can contain many vertices, and a line is only returned once a merged
path exists for the stop — there are no placeholder lines. Coordinates are ordered source→target
(the direction in `line_type`).

| Property | Type | Description |
| --- | --- | --- |
| `object_type` | string | `"line"` |
| `line_type` | string | `parking_to_entrance` (the dashed walking path) · `waypoint_to_parking` (the approach) |
| `path_source` | string | `"legs"` — routed over real GPS traces · `"straight"` — straight-line placeholder geometry |
