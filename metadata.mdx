...
| `getMetadataTemplate(name)` | `Map?` | Get template by name |
...
| `appendToMetadataTag(tag, data)` | `Unit` | Append to tagged metadata |
| `getMetadataByTag(tag)` | `Map?` | Get tagged metadata |
