...
The `StandardMetadata` class contains the following fields. All fields are strings, and all are
optional: an omitted or `null` field is uploaded as an empty string, which is what passing `""` has
always produced. Supply the ones your event actually has.
