...
_type: FLOAT (Degrees) |_ Latitude and longitude coordinates of the entrance or parking place.
) 
#### Position Type
_type: STRING |_ This string specifies whether the position is a parking spot or an entrance.

#### Rank
_type: INTEGER |_ This integer represents the rank of an entrance or parking place. truemetrics groups entrances and parking places based on a shared merge identifier (typically the address identifier). In some configurations, multiple entrances or parking places may be exported for the same merge identifier. The rank determines their order and priority within that group.
#### Timestamp
