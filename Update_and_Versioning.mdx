...
Example Use Case:
If truemetrics initially sent you an entrance location that was later determined to be incorrect (e.g., through manual verification), the daily export will include a row with the same identifier but with lat = 0 and lon = 0.
You should interpret this as a deletion request and remove or ignore the corresponding record in your system.
...
### [outdated]  Full Export for Positions

Earlier versions of truemetrics also offered full export.

###  Complete Export for Stops
For each delivery you have done so far, we provide the associated Stop KPIs for the last 90 days.
...
We are processing all your deliveries at the end of the day (at 00:00 am) and perform the export of the data on the end of the next day (at around 23:00 am). This results in a delay between delivery and export of 24-48 hours. 
